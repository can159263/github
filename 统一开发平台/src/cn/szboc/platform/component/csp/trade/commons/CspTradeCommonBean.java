...
/**
 * CSP交易,CTG通信域,通用头部(300字节)
 * 头部字段全部以common打头,子类的上行字段使用up打头,下行字段使用down打头
 * 转换注解MessageField不要出现在setUpXxx,getDownXxx中,虽然组件不一定报错,但肯定不合语义
 */
@MessageBean(
	Target = To.BOTH,
	Charset = MsgCharset.GB18030,
	ExpectedReadSize = 300,
	ExpectedWriteSize = 300
)
public class CspTradeCommonBean {
